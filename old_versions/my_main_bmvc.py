...
def forward(data_loader, model, bin_model, criterion,  epoch=0, training=True, optimizer=None, 
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if training:
            ...
            # Update the binarized model for evaluation 
            with torch.no_grad():
                for n, p in bin_model.named_parameters():
                    if if_binary(n):
                        p.data.copy_(hardtanh_params[n].sign())
                    else:
            ...
            for n, p in model.named_parameters():
                if if_binary(n):
                    y = p.data/delta
                    eta_vec = torch.ones_like(y)
                    grad = p.grad.data
                    final_grad = copy.deepcopy(p.grad.data)
                    ...
                    grad[curr_mask] = eta_vec[curr_mask] * grad[curr_mask]
