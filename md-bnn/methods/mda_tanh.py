"""MD-TANH method for NN Quantization
"""
...
def train_step(args, model, device, data, target, optimizer, criterion):
...
def evaluate(args, amodel, model, device, loader, beta=1.0, training=False, summary_writer=None, iterations=None):
    ...
    if training:
        ...
        if summary_writer is not None:
            for name, param in model.named_parameters():
                summary_writer.add_histogram(name, param.clone().cpu().data.numpy(), iterations, bins=1000)
...
def init_weights(model, device, args, xavier=False):
    for p in model.parameters():
        if xavier and len(p.size()) >= 2:
            nn.init.xavier_uniform_(p)
        else:
            nn.init.normal_(p, std=0.1)
    if args.optimizer == 'MDA_TANH_SGD' or args.optimizer == 'MDA_TANH_ADAM':
        dotanh(args, model, beta=1.0)
...
def setup_and_run(args, criterion, device, train_loader, test_loader, val_loader, logging, results, summary_writer):
    ...
    if args.optimizer == 'MDA_TANH_ADAM':
        optimizer = coptim.MDAadam(model.parameters(), projection_fn='tanh', lr=args.learning_rate, weight_decay=args.weight_decay
                                   , beta_mda=beta)
    else:
    ...
    init_weights(model, device, args, xavier=True)
    if not args.eval:
        logging.info(model)
    ...
    if not args.eval:
        ...
        for e in range(start_epoch + 1, args.num_epochs):
            for i, (data, target) in enumerate(train_loader):
                ...
                l = train_step(args, model, device, data, target, optimizer, criterion)
    
                if i % args.log_interval == 0:
                    acc1, acc5 = evaluate(args, amodel, model, device, val_loader, beta, training=True,
