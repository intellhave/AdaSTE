...
def train_step(args, amodel, model, device, data, target, optimizer, criterion, beta=1.):
    ...
    # gradient wrt to auxparams (g_u * (d P(tu)/d (tu))
...
def setup_and_run(args, criterion, device, train_loader, test_loader, val_loader, logging, results, summary_writer):
    ...
    logging.info('Evaluating fractional pgd-simplex-net on the {0} set...'.format(args.eval_set))
    st = timer()                
    ...
    doround(model, device, scheme=args.rounding)
    logging.info('Evaluating discrete pgd-simplex-net on the {0} set...'.format(args.eval_set))
    st = timer()                
    if args.eval_set == 'TRAIN':
        acc1, acc5 = evaluate(args, amodel, model, device, train_loader)
    else: 
