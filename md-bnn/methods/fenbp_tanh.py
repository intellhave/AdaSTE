""" GD-TANH or MD-TANH-STABLE Code
"""
    ...
    for i, (name, p) in enumerate(model.named_parameters()):
        ...
        if args.tanh:
            if args.quant_levels == 2:
                # w = p.data  # use already projected
                # ww = 1. - w.pow(2)  # 1 - tanh^2(x)
                # ww.mul_(beta)
                # p.grad.data = g.mul(ww) # hadamard

                y = p.data * beta
                mask_pos_grad = p.grad.data > 1e-3
                mask_neg_grad = p.grad.data < -1e-3
                mask_pos_x = p.data > 1.0/beta + 1e-3
                mask_neg_x = p.data < -1.0/beta - 1e-3

                curr_mask = mask_neg_x & mask_neg_grad
                p.grad.data[curr_mask] = -2/(y[curr_mask]-1) * g[curr_mask]
                ...
                ww = 0.5 * (2. - torch.tanh(beta*(w + 0.5)).pow(2) - torch.tanh(beta*(w - 0.5)).pow(2))  # 0.5 (1 - tanh^2(x1) + 1 - tanhh^2(x2))
                ww.mul_(beta)
                p.grad.data = g.mul(ww) # hadamard
            else:
                assert(0)
...
def dotanh(args, model, beta=1.):
    """ tanh projection
    """
    for i, (name, p) in enumerate(model.named_parameters()):
        if args.quant_levels == 2:  # binary
            # p.data = torch.tanh(p.data * beta)
            p.data = F.hardtanh(p.data * beta)
        elif args.quant_levels == 3:    # ternary, shifted tanh
            p.data = 0.5 * (torch.tanh(beta*(p.data + 0.5)) + torch.tanh(beta*(p.data - 0.5)))
        else:
            assert(0)


def dotanh_inv(args, model, beta=1.):
    """ tanh projection
    """
    epsilon = 1e-20
    for i, (name, p) in enumerate(model.named_parameters()):
        if args.quant_levels == 2:
            # binary
            p.data = (1/beta) * torch.tanh(0.5 * torch.log((1.0 + p.data) / (1.0 - p.data + epsilon)))
...
def train_step(args, amodel, model, device, data, target, optimizer, criterion, beta=1.):
    ...
    # restore aux-weights
    if not args.zeroone and not args.tanh:
        doclamp(args, model)  # <-1 or >1 is irrelevant for sign()
...
def setup_and_run(args, criterion, device, train_loader, test_loader, val_loader, logging, results, summary_writer):
    ...
    start_epoch = -1
    beta = 800    # discrete forcing scalar, used only for softmax based projection
    ...
    if not args.eval:
        ...
        for e in range(start_epoch + 1, args.num_epochs):
            ...
            logging.info('Epoch: {0},\t Iter: {1},\t Loss: {loss:.5f},\t Val-Acc1: {acc1:.2f} '
                    '(Best: {best:.2f}),\t Val-Acc5: {acc5:.2f}'.format(e, i, 
                        loss=epoch_loss/300, acc1=acc1, best=BEST_ACC, acc5=acc5))
            results.add(epoch=e, iteration=i, train_loss=epoch_loss/500, val_acc1=acc1, best_val_acc1=BEST_ACC)
